...
function perform_backup
{
  ...
  # echo "3 = $3"
  # echo "${2}/${3}.tar.gz"
  tar cf - ${1} -P | pv -s ${SIZE_TO_BACKUP} | gzip | \
    gpg --homedir /media/truecrypt21/pgp/keys_for_epm_files/ \
     -r Backup --output "${2}"/"${3}".tar.gz.gpg --encrypt
  echo "Done ${1}"
}
...
BACKED_DIR=/mnt/big/out/develop/java/
# tar cf - ${BACKED_DIR} -P | pv -s $(du -sb /${BACKED_DIR} | awk '{print $1}') | gzip > ${ARCHIVE_DIR}/big-files.tar.gz
perform_backup "${BACKED_DIR}" "${ARCHIVE_DIR}" "java_test"
